...
<%
   sql="SELECT burrowingID, returnDate FROM BurrowingProcess"
   set rs = ConAct.Execute(sql)
%>

<form method="post" action="returnBook.asp">
<input type="hidden" name="burrowingID" value="<%= rs("burrowingID") %>">
Choose return date <input type="date" name="returnDate" value="<%= rs("returnDate") %>" required/><br>
