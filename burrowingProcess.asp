...
<form method="post" action="burrowingProcessAction.asp">
Burrower Name:  <select name="burrowerName" required><br>
<!--    select     -->
<%
    sql2 = "SELECT burrowerName FROM Burrowers" %>
    <%=sql2 %>

    <option value=""></option>
  <%  set rs2 = ConAct.Execute(sql2)
       while not rs2.eof
        burrowerName = rs2("burrowerName")
       %>
        <option value="<%=burrowerName %>"> <%=burrowerName %> </option>
        <%rs2.MoveNext
    Wend
    rs2.Close
%>
</select> <br>
Burrow Date: <input type="date" name="burrowDate" id="burrowDate" required/><br>
Return Date: <input type="date" name="returnDate" id="returnDate" onclick="updateBookList();" required/><br>
Book Name: <select name="bookName" id="book-list" required>
<!--    select     -->
<!--<%
    sql2 = "SELECT bookName FROM Books where availability = 1" %>
    <%=sql2 %>
    
    <option value=""></option>
  <%  set rs2 = ConAct.Execute(sql2)
       while not rs2.eof
        bookName = rs2("bookName")
       %>
...
<input type="submit" value="submit" />
    ...
    <script>
        ...
        var borrowingDate = document.getElementById("burrowDate").value;
        ...
        var url = 'BookNameAjax.asp?burrowDate=' + burrowDate + '&returnDate=' + returnDate;
      

        xhr.open('GET', url, true);
        xhr.overrideMimeType('text/xml; charset=windows-1256');
        xhr.send();
        }
    </script>
</body>
